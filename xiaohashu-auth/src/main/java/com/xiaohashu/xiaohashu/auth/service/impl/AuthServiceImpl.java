...
import com.xiaohashu.xiaohashu.auth.rpc.UserRpcService;
        ...
        switch (loginTypeEnum) {
            case VERIFICATION_CODE: // 验证码登录
                ...
                if (Objects.isNull(userByPhone)) {
                    ...
                    // 那么就是登录，将用户-角色信息录入Redis中
                    // 获取到该用户的所有角色
                    // TODO 先将这个直接设置为1L，之后再进行更新
                }
                break;
            case PASSWORD: // 密码登录
                // todo
                ...
                FindUserByPhoneRespDTO userByPhone1 = userRpcService.findUserByPhone(phone);
                ...
                // 拿到密文密码
                String password1 = userByPhone1.getPassword();

                // 匹配密码是否一致
                boolean matches = passwordEncoder.matches(password, password1);

                // 如果不正确，抛出业务异常，提示用户名或密码不正确
                if (!matches) {
