...
import com.quanxiaoha.framework.common.utils.DateUtils;
...
public class NoteServiceImpl implements NoteService {
    ...
    @Resource
    private Cache<Long,String> caffeineCache;
    ...
    public Response<FindNoteDetailRespVO> findNoteDetail(FindNoteDetailReqVO findNoteDetailReqVO) {
        ...
        // 先从本地缓存中查询
        String findNoteDetailRspVOStrLocalCache = caffeineCache.getIfPresent(noteId);
        ...
        FindNoteDetailRespVO findNoteDetailRspVO = null;
        try {
        ...
        FindNoteDetailRespVO finalFindNoteDetailRspVO = findNoteDetailRspVO;
        ...
        threadPoolTaskExecutor.submit(() -> {
            ...
            caffeineCache.put(noteId,Objects.isNull(finalFindNoteDetailRspVO) ? "null" : noteDetailJson1);
        });

        return Response.success(finalFindNoteDetailRspVO);
    }
    ...
    public Response<?> updateNote(UpdateNoteReqVO updateNoteReqVO) {
        ...
        // 判断权限：非笔记发布者不允许更新笔记
        if (!Objects.equals(loginUserId, noteDO1.getCreatorId())) {
        ...
        // 笔记内容是否更新成功
        boolean isUpdateContentSuccess = true;
        // 无内容的笔记改为无内容的笔记（什么都不干，但是逻辑上需要进行排除） 无内容的笔记改为有内容的笔记（生成uuid保存）
        // 有内容的笔记改为无内容的笔记（删除KV存储） 有内容的笔记改为有内容的笔记（覆盖新笔记存储）
        ...
        } else if (StringUtils.isBlank(content) && !StringUtils.isBlank(contentUuid)){
    ...
    public void deleteNoteLocalCache(Long noteId) {
        caffeineCache.invalidate(noteId);
    }
    ...
    public Response<?> deleteNote(DeleteNoteReqVO deleteNoteReqVO) {
        ...
        // 构建消息对象，并将 DTO 转成 Json 字符串设置到消息体中
        Message<String> messages = MessageBuilder.withPayload(JsonUtils.toJsonString(noteOperateMqDTO))
                .build();
        ...
        // 异步发送 MQ 消息，提升接口响应速度
        rocketMQTemplate.asyncSend(destination, messages, new SendCallback() {
            @Override
            public void onSuccess(SendResult sendResult) {
    ...
    public Response<?> likeNote(LikeNoteReqVO likeNoteReqVO) {
        ...
        // 1. 校验被点赞的笔记是否存在,若都不存在会报错，其中一个存在则不报错，并且进行缓存
        Long createorId = checkNoteIsExist(noteId);
        ...
        switch (noteLikeLuaResultEnum) {
            ...
            case NOT_EXIST -> {
                ...
                Integer count = noteLikeDOMapper.selectCountByUserIdAndNoteId(userId, noteId);
                ...
                // 目标笔记已经被点赞
                if (count > 0) {
                    // 如果被点赞了，就异步去初始化布隆过滤器，如果没被点赞，就使用同步先将布隆过滤器进行初始化
                    // 异步初始化布隆过滤器
                    threadPoolTaskExecutor.submit(() ->
    ...
    public Response<?> collectNote(CollectNoteReqVO collectNoteReqVO) {
        ...
        if (Objects.equals(result, NoteCollectLuaResultEnum.NOT_EXIST.getCode())) {
            ...
            // Lua 脚本路径
            script3.setScriptSource(new ResourceScriptSource(new ClassPathResource("/lua/note_collect_update_zset.lua")));
            // 返回值类型
            script3.setResultType(Long.class);

            redisTemplate.execute(script3, Collections.singletonList(userNoteCollectZSetKey), noteId,DateUtils.localDateTime2Timestamp(now));
        }
